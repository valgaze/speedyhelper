...
export default class Web extends Command {
  ...
  static flags = {
    ...
    skip: flags.boolean({char: 's', description: 'Skip questions and just launch web ui (you can enter token + room later)'}),
  ...
  async run() {
    ...
    this.log('Note: Bot tokens currently can only access mentions (1-1 conversations), not entire conversations or rooms')
    if (!flags.skip) {
